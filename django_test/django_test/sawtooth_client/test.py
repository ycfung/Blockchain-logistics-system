...
ODER_NAMESPACE = hashlib.sha512('ordder_state'.encode("utf-8")).hexdigest()[0:4]
...
def sign_up(mobile, key):
    if (key is None):
        temp_client = Client(rest_api_addr, pri_key_str=getattr())
    else:
...
def initial_transfer(amount, pub_key):
    root.send_txn(json.dumps({'action': 'transfer', 'coin': amount, 'acceptor': pub_key}).encode(), anotherUser=pub_key)
...
def fetch_package(user_private_key, order_number, mobile):
    ...
    user.send_txn(json.dumps({'action': 'fetch', 'order_number': order_number}).encode(), order_numbers=[order_number],
                  mobiles=[mobile])
...
def new_request(user_private_key, order_number, coin, destination, mobile):
    user = Client(rest_api_addr, pri_key_str=user_private_key)
    user.send_txn(
        json.dumps({'action': 'apply', 'order_number': order_number, 'coin': coin, 'destination': destination}).encode()
...
def accept_request(user_private_key, order_number, mobile):
    user = Client(rest_api_addr, pri_key_str=user_private_key)
    user.send_txn(json.dumps({'action': 'accept', 'order_number': order_number}).encode(), order_numbers=[order_number],
                  mobiles=[mobile])
...
# 确认委托完成
def confirm_request(user_private_key, order_number, recipient_public_key, mobile):
    user = Client(rest_api_addr, pri_key_str=user_private_key)
